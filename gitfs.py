...
class TreeNode(Node):
    ...
    def del_git_node(self, name):
        del self.git_object[name]
        return True
    ...
    def rename_node(self, oldname, newname):
        mode, git_object = self.get_git_node(oldname)
        self.add_git_node(newname, mode, git_object)
        self.del_git_node(oldname)
...
class Snapshot(object):
    ...
    def commit(self, name, email='devnull@example.com', message='wip', parents=None):
        if not self.is_head:
            return False
...
def example():
    ...
    s = Snapshot(r)
    root = s.root()
    root.rename_node('dir1', 'RENAMED')
    root.save()
    print s.commit('testuser', message='Rename dir1 -> RENAMED').id
    dir1 = root.get_node('RENAMED')
    dir2 = dir1.get_node('dir2')
    print '$ ls dir1/dir1'
    pprint(dir2.nodes())
